...
export function convert_sheet_to_objects(table, category, relevant) {
  let keys = table[0];
  let result = [];
  table.slice(1, table.length).forEach((row) => {
    let temp = {};
    keys.forEach((key, idx) => { temp[key] = row[idx]; });
    if (temp["Category"] === category && (temp["Relevant"] === "relevant" || relevant === "all")) { result.push(temp); }
  });
  return result;
}
...
export function get_gsheet_data(sheet_id, cell_range, callback) {
  gapi.client.sheets.spreadsheets.values
    ...
    .then( 
      (res) => { callback(res.result.values) },
      (err) => { console.log(err) }
    );
}
      ...
      gapi.client.init(config)
        .then(
          (res) => { 
            ...
            if (!login_check) { gapi.auth2.getAuthInstance().signIn() }
            else { setPage("screen") }
           },
